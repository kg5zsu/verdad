...
int browserLineHeightForChoice(const WrappingInputChoice* choice) {
    if (!choice) return 20;
...
int measureEntryWidth(const WrappingInputChoice* choice, const std::string& label) {
    if (!choice) return choice_popup::kPopupDefaultMinColumnW;
...
class WrappingInputChoicePopupWindow : public Fl_Double_Window {
public:
    WrappingInputChoicePopupWindow(WrappingInputChoice* owner, int X, int Y, int W, int H)
...
private:
    WrappingInputChoice* owner_ = nullptr;
};

class WrappingInputChoiceColumnBrowser : public Fl_Hold_Browser {
public:
    WrappingInputChoiceColumnBrowser(WrappingInputChoice* owner,
                                     int columnIndex,
        ...
        callback(WrappingInputChoice::onPopupPicked, owner_);
        box(FL_BORDER_BOX);
...
WrappingInputChoice::WrappingInputChoice(int X, int Y, int W, int H, const char* label)
    : Fl_Input_Choice(X, Y, W, H, label) {
    choice_popup::registerPopupOwner(this);
}

WrappingInputChoice::~WrappingInputChoice() {
    hidePopup();
    destroyPopup();
    choice_popup::unregisterPopupOwner(this);
}

void WrappingInputChoice::setPopupInitialIndex(int index) {
    popupInitialIndex_ = index;
}

bool WrappingInputChoice::consumeMenuSelectionCallback() {
    const bool wasMenuSelection = menuSelectionPending_;
    menuSelectionPending_ = false;
    return wasMenuSelection;
}

int WrappingInputChoice::handle(int event) {
...
void WrappingInputChoice::resize(int X, int Y, int W, int H) {
...
void WrappingInputChoice::destroyPopup() {
...
bool WrappingInputChoice::eventInMenuButton() const {
...
bool WrappingInputChoice::supportsCustomPopup() const {
    Fl_Menu_Button* button = const_cast<WrappingInputChoice*>(this)->menubutton();
    if (!button || !button->menu() || button->size() <= 0) return false;

    for (int i = 0; i < button->size(); ++i) {
        const Fl_Menu_Item& item = button->menu()[i];
...
bool WrappingInputChoice::buildPopup() {
    menubutton()->menu_end();
    popupEntries_.clear();

    for (int i = 0; i < menubutton()->size(); ++i) {
        const Fl_Menu_Item& item = menubutton()->menu()[i];
        ...
        for (int column = 0; column < columnCount; ++column) {
            ...
            while (entryIndex < popupEntries_.size() &&
                ...
                const int desiredWidth =
                    measureEntryWidth(this, popupEntries_[entryIndex].label);
    ...
    popupWindow_ = new WrappingInputChoicePopupWindow(this, popupX, popupY, popupW, 1);
    popupWindow_->begin();
    ...
    for (size_t column = 0; column < popupColumnEntries_.size(); ++column) {
        ...
        auto* browser = new WrappingInputChoiceColumnBrowser(
            this,
    ...
    for (auto* browser : popupBrowsers_) {
        auto* popupBrowser = static_cast<WrappingInputChoiceColumnBrowser*>(browser);
        ...
        desiredBodyH = std::max(desiredBodyH, visibleBodyH);
    ...
    for (size_t i = 0; i < popupBrowsers_.size(); ++i) {
        ...
        choice_popup::positionBrowserForOpen(browser, selectedLineForColumn(static_cast<int>(i)));
    }
...
bool WrappingInputChoice::showPopup() {
...
void WrappingInputChoice::hidePopup() {
    if (Fl::grab() == popupWindow_) {
        Fl::grab(nullptr);
    }
    if (choice_popup::activePopupOwner() == this) {
...
bool WrappingInputChoice::popupVisible() const {
    return popupWindow_ && popupWindow_->shown() &&
           choice_popup::activePopupOwner() == this;
}

bool WrappingInputChoice::popupContainsRootPoint(int rootX, int rootY) const {
...
int WrappingInputChoice::selectedMenuIndex() const {
    Fl_Menu_Button* button = const_cast<WrappingInputChoice*>(this)->menubutton();
    if (popupInitialIndex_ >= 0 && button && popupInitialIndex_ < button->size()) {
        return popupInitialIndex_;
    }
    return button ? button->value() : -1;
}

int WrappingInputChoice::currentPopupColumn() const {
    const int currentIndex = selectedMenuIndex();
...
int WrappingInputChoice::selectedLineForColumn(int columnIndex) const {
    if (columnIndex < 0 ||
...
void WrappingInputChoice::focusColumn(int columnIndex, int preferredLine) {
...
void WrappingInputChoice::moveFocus(int deltaColumns) {
...
void WrappingInputChoice::selectBrowserLine(Fl_Hold_Browser* browser, int line) {
    ...
    if (!entry.active || !menubutton() || !menubutton()->menu() ||
        entry.menuIndex < 0 || entry.menuIndex >= menubutton()->size()) {
        return;
    }

    const Fl_Menu_Item* item = &menubutton()->menu()[entry.menuIndex];
    popupInitialIndex_ = entry.menuIndex;
    hidePopup();

    menubutton()->picked(item);
    const char* menuText = menubutton()->text();
    const char* currentText = value();
    if (!menuText) menuText = "";
    if (!currentText) currentText = "";

    if (std::strcmp(currentText, menuText) == 0) {
...
int WrappingInputChoice::columnIndexForBrowser(const Fl_Hold_Browser* browser) const {
    for (size_t i = 0; i < popupBrowsers_.size(); ++i) {
...
void WrappingInputChoice::onPopupPicked(Fl_Widget* widget, void* data) {
    auto* self = static_cast<WrappingInputChoice*>(data);
...
Fl_Widget* WrappingInputChoice::popupAnchorWidget() const {
    return const_cast<WrappingInputChoice*>(this);
}

void WrappingInputChoice::showOwnedPopup() {
    showPopup();
}

void WrappingInputChoice::hideOwnedPopup() {
    hidePopup();
}

bool WrappingInputChoice::popupIsVisible() const {
    return popupVisible();
}

void WrappingInputChoice::takeOwnedFocus() {
    if (Fl::visible_focus()) Fl::focus(menubutton());
}
